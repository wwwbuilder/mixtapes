var Creditly = (function() {
      ...
      $(selector).keypress(function(e) {
        ...
        if (shouldProcessInput(e, maximumLength, selector)) {
          ...
          if (inputValue.length >= 2) {
            ...
            $('#testting').val(newInput)
            var inp = newInput;
            var expArray = inp.split( '/' );
            var expmm = ( expArray[ 0 ] ).trim();
            var expyy = ( expArray[ 1 ] ).trim();
            if ( expmm.length == 2 && expyy.length == 2){
            $('#exp_monthh').val(expmm)
            $('#exp_yearh').val(expyy)
            console.log(expmm)
            console.log(expyy)
            }
  ...
  var CardTypeListener = (function() {
    var determineCardType = function(value) {
      if (/^(34|37)/.test(value)) {
        return '<img src=\"'+STATIC_URL+'img/ae.png\" />';
      } else if (/^4/.test(value)) {
        return '<img src=\"'+STATIC_URL+'img/visa.png\" />';
      } else if (/^5[0-5]/.test(value)) {
        return '<img src=\"'+STATIC_URL+'img/mastercard.png\" />';
      } else if (/^(6011|622|64[4-9]|65)/.test(value)) {
        return '<img src=\"'+STATIC_URL+'img/discover.png\" />';
      } else {
        return "";
      }
    };
    ...
    var changeCardType = function(numberSelector, cardTypeSelector) {
      $(numberSelector).on("changed_input keypress keydown keyup", function(e) {
        ...
        $(cardTypeSelector).html(cardType);
      });
    };
