...
function deleteFile (filetype, object_id) {
	...
	if (confirm('Delete ' + ft + ': This can not be undone. Proceed?')){
		$.ajax({
			...
			type: 'delete',
			// Seems easier to get id from url path in Django when using HTTP DELETE,
			// because it only constructs request dicts for GET and POSTS
			//data: {'id': object_id},
			success: function (data) {
	...
	holder.find('tbody tr').each(function(i, v){
		...
		if ($(v).find('textarea, input').parsley().validate() == true && doublecheck){
			...
			if (filetype == 'images') {
				...
				endpoint_url = endpoint + object_id;
