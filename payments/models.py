...
#from django.conf.settings import DEFAULT_PLAN,INVOICE_FROM_EMAIL,PAYMENTS_PLANS,\
                             #plan_from_stripe_id, SEND_EMAIL_RECEIPTS,TRIAL_PERIOD_FOR_USER_CALLBACK, PLAN_QUANTITY_CALLBACK
from signals import cancelled, card_changed, subscription_made,webhook_processing_error, WEBHOOK_SIGNALS
...
def plan_from_stripe_id(stripe_id):
    for key in settings.PAYMENTS_PLANS.keys():
        if settings.PAYMENTS_PLANS[key].get("stripe_plan_id") == stripe_id:
            return key
...
class Customer(StripeObject):
    ...
    def has_active_subscription(self):
        try:
            subscriptions = self.current_subscription.filter()
            if subscriptions:
                return subscriptions[0].is_valid()
            else:
                return False
        except CurrentSubscription.DoesNotExist:
            return False
    
    def has_active_publisher_subscription(self):
        publisher_subscription = self.current_subscription.filter(plan__startswith='premium-publisher')
        if publisher_subscription:
            return publisher_subscription.latest('id').is_valid()
        else:
            return False
        
    def has_active_member_subscription(self):
        member_subscription = self.current_subscription.filter().exclude(plan__startswith='premium-publisher')
        if member_subscription:
            return member_subscription.latest('id').is_valid()
        else:
            return False    
    
        
    def publisher_subscription(self):
        subscriptions = self.current_subscription.filter(plan__startswith='premium-publisher')
        if subscriptions:
            return subscriptions.latest('id')
   
    def member_subscription(self):
        subscriptions = self.current_subscription.filter().exclude(plan__startswith='premium-publisher')
        if subscriptions:
            return subscriptions.latest('id')    
    ...
    def member_cancel(self, at_period_end=True):
        try:
            current = self.member_subscription()
        except CurrentSubscription.DoesNotExist:
            return
        sub = self.stripe_customer.cancel_subscription(
            at_period_end=at_period_end
    ...
    def create(cls, user, card=None, plan=None, charge_immediately=True):
        if card and plan:
            plan = settings.PAYMENTS_PLANS[plan]["stripe_plan_id"]
        #elif settings.DEFAULT_PLAN:
            #plan = settings.PAYMENTS_PLANS[DEFAULT_PLAN]["stripe_plan_id"]
        else:
            plan = None

        trial_end = None
        if settings.TRIAL_PERIOD_FOR_USER_CALLBACK and plan:
            trial_days = settings.TRIAL_PERIOD_FOR_USER_CALLBACK(user)
    ...
    def subscribe(self, plan, quantity=None, trial_days=None,
        ...
        if quantity is None:
            if settings.PLAN_QUANTITY_CALLBACK is not None:
                quantity = settings.PLAN_QUANTITY_CALLBACK(self)
            else:
                quantity = 1
        cu = self.stripe_customer
        ...
        subscription_params["plan"] = settings.PAYMENTS_PLANS[plan]["stripe_plan_id"]
...
class Invoice(models.Model):
    ...
    @classmethod
    def handle_event(cls, event, send_receipt=settings.SEND_EMAIL_RECEIPTS):
    ...
    def send_receipt(self):
        if not self.receipt_sent:
            ...
            num_sent = EmailMessage(
                ...
                from_email=settings.PAYMENTS_INVOICE_FROM_EMAIL
            ).send()
